...
public class KNN_DistanceLearn2 extends Algorithm {
	...
	int train(DatabaseManager m_DatabaseManager) {
		...
		File File_temp1=new File("F:" +File.separator+ "workspace" + File.separator+"tempfiledealdistance");
		if(File_temp1.exists())
		{
			File [] m_file=File_temp1.listFiles();
			for(File one:m_file)
			{
				one.delete();
			}
		...
		for(Integer temp_map_id:map_id)
		{
			...
			try {
				File temp_distance=new File("F:"+File.separator+"workspace"+File.separator+"点之间的物理距离"+temp_map_id+".txt");
				...
				{
					ResultSet  rs1=m_DatabaseManager.get_ResultSet_from_wifi_sample(temp_Integer_from);
					int temp_x=0;
				   	int temp_y=0;
		            try {
					...
					{
						ResultSet  rs2=m_DatabaseManager.get_ResultSet_from_wifi_sample(temp_Integer_to);
						int temp_x2=0;
					   	int temp_y2=0;
			            try {
							...
							if(rs2.next())
							  {
							   	temp_x2=rs2.getInt("x");
							   	temp_y2=rs2.getInt("y");
							  }
							rs2.close();
						} catch (SQLException e) {
							// TODO Auto-generated catch block
						...
						double distance_from_to=Math.sqrt(Math.pow((temp_x-temp_x2), 2)+Math.pow((temp_y-temp_y2), 2));
						dd+=""+distance_from_to+"   ";
					}
					bw.write(dd);
					bw.newLine();
			...
			double [] delt=new double[dimension_list.size()];
			for(int i=0;i<dimension_list.size();i++)
			{
				P[i]=1.0/dimension_list.size();
			}
			////iter
			...
			int iter=0;
			System.out.println(""+(before-sum_all2));
			while(Math.abs(before-sum_all2)>0.0001)
			{
				before=sum_all2;
				for(int i=0;i<dimension_list.size();i++)
				{
				...
				sum_all2=0;
				BufferedReader in_txt_D = null;
				
				try {
					in_txt_D = new BufferedReader(new FileReader("F:"+File.separator+"workspace"+File.separator+"点之间的物理距离"+temp_map_id+".txt"));
				...
				for(Integer from:all_sample_id)
				{
					...
					BufferedReader in_txt_WIFI;
					try {
						
						in_txt_WIFI = new BufferedReader(new FileReader("F:" +File.separator+ "workspace" + File.separator+"tempfile"+File.separator+from+"_id_all.txt"));
						String con="";
						try {
							String conD=in_txt_D.readLine();
							
							String []Dis=conD.split("\\s+");
							double sum_dis=0;
							for(int i=0;i<Dis.length;i++)
							{
								sum_dis+=Double.parseDouble(Dis[i]);
							}
							double sum_wifi=0;
							while((con=in_txt_WIFI.readLine())!=null)
							{
								String [] value=con.split("\\s+");
								double sum=0;
								for(int i=0;i<value.length;i++)
								{
									sum+=1*Double.parseDouble(value[i]);
								}
								sum_wifi+=sum;
							}
							in_txt_WIFI.close();
							...
							while((con=in_txt_WIFI.readLine())!=null)
							{
								...
								for(int i=0;i<value.length;i++)
								{
									sum+=P[i]*Double.parseDouble(value[i]);
								}
								double dist=Double.parseDouble(Dis[j]);
								double one_value=sum/sum_wifi-dist/sum_dis;
								for(int i=0;i<value.length;i++)
								{
									delt[i]+=one_value*Double.parseDouble(value[i])/sum_wifi;
								}
								
								sum_all2+=one_value*one_value;
				...
				try {
					File temp_distance=new File("F:"+File.separator+"workspace"+File.separator+"tempfiledealdistance"+File.separator+"map_id_"+temp_map_id+"_处理后点之间的距离"+iter+".txt");
					if(temp_distance.exists())
					{
						temp_distance.delete();
						temp_distance.createNewFile();
					...
					for(Integer temp_Integer_from:all_sample_id)
					{
						...
						for(Integer temp_Integer_to:all_sample_id)
						{
							...
							for(String one:dimension_list)
							{
								distance_from_to+=getdistance(from.get(one),to.get(one));//Math.pow(from.get(one)-to.get(one), 2);
								i++;
							}
							all+=distance_from_to;
						}
						for(Integer temp_Integer_to:all_sample_id)
						{
							HashMap<String ,Integer> to=m_DatabaseManager.get_wifi_info(temp_Integer_to);
							...
							for(String one:dimension_list)
							{
								distance_from_to+=P[i]*getdistance(from.get(one),to.get(one));//Math.pow(from.get(one)-to.get(one), 2);
								i++;
							}
							dd+=""+distance_from_to/all+"   ";
						}
						bw.write(dd);
						bw.newLine();
						bw.flush();
				...
				for(int i=0;i<dimension_list.size();i++)
				{
					//delt[i]=delt[i]/delt_sum;
					System.out.print(" "+delt[i]+"\t");
				}
				boolean flag=true;
				for(int i=0;i<dimension_list.size();i++)
				{
					if(Math.abs(delt[i])>0.001)
						flag=false;
				}
				if(flag)
					break;
				System.out.println();
				System.out.println("Math.abs(before-sum_all2):   "+Math.abs(before-sum_all2));
				iter++;
				//////////////////
			}
			...
			for(Integer temp_Integer_from:all_sample_id)
			{
				...
				{
					m_DatabaseManager.insert_into_cluster_centre_wifi_info(cluster_id, name, one.get(name), P[i], 0, 0, 0);
		       		i++;
				}
			}
